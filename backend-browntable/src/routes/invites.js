...
router.post('/invite-member', authMiddleware, async (req, res) => {
  try {
    ...
    const inviteLink = `${process.env.FRONTEND_URL || 'http://localhost:5173'}/join?code=${group.inviteCode}`;
...
router.post('/invite-user', authMiddleware, async (req, res) => {
  try {
    ...
    const isAlreadyMember = group.groupMembers.some(member => member.userId === invitedUser.id);
    if (isAlreadyMember) {
      return res.status(400).json({
        success: false,
    ...
    const hasPendingInvite = invitedUser.pendingInvites.some(invite => invite.groupId === groupId);
    if (hasPendingInvite) {
      return res.status(400).json({
        success: false,
...
router.post('/join', authMiddleware, async (req, res) => {
  try {
    ...
    // Check if user is already a member
    const isAlreadyMember = group.groupMembers.some(member => member.userId === user.id);
    if (isAlreadyMember) {
      return res.status(400).json({
        success: false,
    ...
    const inviteIndex = user.pendingInvites.findIndex(invite => invite.groupId === group.id);
    if (inviteIndex !== -1) {
...
router.post('/accept/:groupId', authMiddleware, async (req, res) => {
  try {
    ...
    const inviteIndex = user.pendingInvites.findIndex(invite => invite.groupId === groupId);
    if (inviteIndex === -1) {
      return res.status(400).json({
        success: false,
